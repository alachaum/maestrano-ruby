  ...
  module Connec
    class ClientTest < Test::Unit::TestCase
      ...
      context 'with preset' do
        ...
        context 'get' do
          setup {
            @klass = Maestrano::Connec::Client[@preset]
            @client = @klass.new("cld-123")
          }
          ...
          should "perform the right query" do
            ...
            opts = { foo: 'bar' }
            resp = mock('resp')
            @klass.expects(:get).with(@client.scoped_path(path),@client.default_options.merge(opts)).returns(resp)
            assert_equal resp, @client.get(path,opts)
          end
        end
        ...
        context 'post' do
          ...
          should "perform the right query" do
            ...
            opts = { foo: 'bar' }
            resp = mock('resp')
            @klass.expects(:post).with(@client.scoped_path(path),@client.default_options.merge(body: body.to_json).merge(opts)).returns(resp)
            assert_equal resp, @client.post(path,body,opts)
          end
        end
        ...
        context 'put' do
          ...
          should "perform the right query" do
            ...
            opts = { foo: 'bar' }
            resp = mock('resp')
            @klass.expects(:put).with(@client.scoped_path(path),@client.default_options.merge(body: body.to_json).merge(opts)).returns(resp)
            assert_equal resp, @client.put(path,body,opts)
          end
        end
      end
